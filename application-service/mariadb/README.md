...
## 数据库操作
### 创建数据
> create database | schema [it not exits] db_name
...
### where子句子查询
- 用于比较表达式中的子查询：资产讯仅能返回单个数
	+ `SELECT Name，Age from students where Age>(select avg(Age) from students)`
- in中的子查询：子查询应该单键查询并返回一个或多个值从构成列表
	+ `SELECT Name，Age from students where Age IN(select Age from students)`
- 用于exists
...
- 修改密码：
	+ `set password for`
	+ `update  mysql.user set password=password('you_password') where clause`
		* 不会重读授权表，因此执行`mysql> flush privileges`
	+ `mysqladmin password`
