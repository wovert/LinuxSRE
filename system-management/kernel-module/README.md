...
- --with=<module> 除了**默认的模块之外装载**至initramfs中的模块
- --preload=<module> initramfs所提供的模块需要**预先装载的模块**
...
- 查看：
`# cat /proc/sys/PATH/TO/TO/SOME_KERNEL_FILE`
...
- 立即生效(-p 读取配置文件)
`# sysctl -p [/PATH/TO/CONFIG_FILE]` 默认读取/etc/sysctl.conf
...
### 了解全部硬件设备信息：
`# yum list all | grep hal` CentOS 6
`# hal-device`
...
- 实现清理操作：
`# make clean` 		清理编译生成的绝大多数文件，当会保留config，及编译外部模块所需要的文件
