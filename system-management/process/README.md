...
## 允许同时存在进程最大数目：默认为32768（short int），可以增加到400万个。修改内核文件**/proc/sys/kernel/pid_max**提高上限
...
### 不同主机上：
- RPC：Remote Procedure Call 远程过程调录
- Socket: 一段文件数据发送到另一端
...
## 进程的分类：
- CPU-Bound: CPU密集型，非交互式进程（高清蓝光电影）
- IO-Bound: IO密集型，交互式进程
...
### C/S模式下运行glances命令：
- 服务模式
`glances -s -B IPADDR` IPADDR：本机的某地址，用于监听

- 客户端模式
`glances -c IPADDR` IPADDR: 是远程服务器的地址
